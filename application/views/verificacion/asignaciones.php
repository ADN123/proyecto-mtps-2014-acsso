...
<script>
	$('#id_empleado').change(function(){
		id=$(this).val();
		$('#cont-calendario').load(base_url()+'index.php/verificacion/calendario/'+id+'/1');
		$('#cont-calendario-dia').load(base_url()+'index.php/verificacion/calendario_dia/0/0');
	});
	...
	$('#calendar').fullCalendar({
		...
		eventClick: function(event, jsEvent){	
			fecha_actual=event.id;	
			 <?php if($id_permiso==4) {?>		 
				$('#cont-calendario-dia').load(base_url()+'index.php/verificacion/calendario_dia/<?php echo $id_empleado?>/'+event.id);
			 <?php } else {?>		 
